...
const TIER_CONFIG = {
  bronze: { gradient: 'from-[hsl(25,60%,45%)] to-[hsl(35,70%,55%)]', glow: 'hsl(30 65% 50% / 0.3)', ring: 'hsl(30, 65%, 50%)', bg: 'bg-[hsl(30,60%,50%)]/10', text: 'text-[hsl(30,65%,55%)]', label: 'Bronze' },
  silver: { gradient: 'from-[hsl(220,10%,60%)] to-[hsl(220,15%,80%)]', glow: 'hsl(220 15% 70% / 0.3)', ring: 'hsl(220, 15%, 70%)', bg: 'bg-[hsl(220,15%,70%)]/10', text: 'text-[hsl(220,15%,80%)]', label: 'Silver' },
  gold: { gradient: 'from-[hsl(43,96%,46%)] to-[hsl(43,96%,66%)]', glow: 'hsl(43 96% 56% / 0.4)', ring: 'hsl(43, 96%, 56%)', bg: 'bg-readiness-yellow/10', text: 'text-readiness-yellow', label: 'Gold' },
  platinum: { gradient: 'from-[hsl(265,90%,56%)] to-[hsl(190,95%,60%)]', glow: 'hsl(265 90% 66% / 0.4)', ring: 'hsl(265, 90%, 66%)', bg: 'bg-metric-purple/10', text: 'text-metric-purple', label: 'Platinum' },
};

function AwardMedal({ award, index, earned, locale }: { award: typeof AWARD_DEFINITIONS[0]; index: number; earned: Award | undefined; locale: string }) {
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col items-center gap-3 p-5 rounded-2xl border transition-all duration-300 overflow-hidden ${
        isEarned ? 'border-border/30 bg-card/60' : 'border-border/10 bg-card/20 opacity-40 grayscale'
      }`}
    >
      {isEarned && (
      ...
      <div className="relative">
        <svg width="72" height="72" viewBox="0 0 72 72">
          ...
          {isEarned && (
            <motion.circle cx="36" cy="36" r="30" fill="none" stroke={tier.ring} strokeWidth="3" strokeLinecap="round"
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <motion.div animate={isEarned ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}>
      ...
      {earned && (
        <div className="flex items-center gap-2">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] text-muted-foreground font-mono">
            {new Date(earned.earned_at).toLocaleDateString(locale, { day: 'numeric', month: 'short', year: 'numeric' })}
          </motion.p>
          <ShareAwardCard award={award} earned={earned}>
            <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }} className="p-1 rounded-lg hover:bg-secondary/30 text-muted-foreground hover:text-foreground transition-colors">
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto px-4 py-5 space-y-6">
        ...
        <div className="flex justify-center">
          <div className="relative">
            <svg width="80" height="80" viewBox="0 0 80 80" className="-rotate-90">
              ...
              <motion.circle cx="40" cy="40" r="32" fill="none" stroke="url(#awards-grad)" strokeWidth="4" strokeLinecap="round"
      ...
      {tiers.map(tier => {
        ...
        return (
          <motion.div key={tier} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.1 }} className="space-y-4">
            <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {tierAwards.map((award, i) => (
                <AwardMedal key={award.key} award={award} index={i} earned={earnedMap.get(award.key)} locale={locale} />
              ))}
            </div>
          </motion.div>
        );
