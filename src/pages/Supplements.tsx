...
const CATEGORIES = [
  ...
  { value: 'mineral', label: 'Khoáng chất' },
...
const TIMINGS = [
  ...
  { value: 'pre-workout', label: 'Trước tập' },
  { value: 'post-workout', label: 'Sau tập' },
  { value: 'with meals', label: 'Cùng bữa ăn' },
  { value: 'before bed', label: 'Trước ngủ' },
];
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        ...
        // Update cycle via direct call since hook doesn't include these fields
    ...
    <div className="min-h-screen bg-background">
      ...
      <motion.header initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold"><span className="text-gradient-green">Supplements</span></h1>
        </div>
      </motion.header>
      ...
      <motion.main initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.06 } } }} className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <motion.div variants={fadeUp} className="grid grid-cols-2 gap-4">
          <div className="metric-card text-center">
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Tuần này</p>
          </div>
          <div className="metric-card text-center">
            <Calendar className="w-5 h-5 mx-auto text-metric-purple mb-2" />
            <p className="text-3xl font-mono font-bold">{adherence30d?.rate ?? 0}%</p>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">30 ngày</p>
          </div>
        </motion.div>

        {/* Add button */}
        <motion.div variants={fadeUp} className="flex justify-between items-center">
          <h3 className="text-sm font-semibold">Stack của bạn ({supplements?.length ?? 0})</h3>
          <Dialog open={addOpen} onOpenChange={(o) => { setAddOpen(o); if (!o) { setEditId(null); resetForm(); } }}>
            <DialogTrigger asChild>
              <Button size="sm" className="rounded-xl"><Plus className="w-3.5 h-3.5 mr-1" />Thêm</Button>
            </DialogTrigger>
            <DialogContent className="max-h-[85vh] overflow-y-auto">
              <DialogHeader><DialogTitle>{editId ? 'Sửa Supplement' : 'Thêm Supplement'}</DialogTitle></DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2"><Label>Tên</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="VD: Creatine" /></div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2"><Label>Liều lượng</Label><Input value={form.dose_text} onChange={e => setForm(f => ({ ...f, dose_text: e.target.value }))} placeholder="5g" /></div>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2"><Label>Ghi chú / Chống chỉ định</Label><Input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="VD: Uống cùng bữa ăn" /></div>

                {/* Cycle settings */}
                <div className="border-t border-border/40 pt-4 space-y-3">
                  <Label className="text-xs uppercase tracking-wider text-muted-foreground">Chu kỳ (Cycle)</Label>
                  <Select value={form.cycle_mode} onValueChange={v => setForm(f => ({ ...f, cycle_mode: v }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">Không cycle</SelectItem>
                      <SelectItem value="on_off">On/Off tuần</SelectItem>
                    </SelectContent>
                  </Select>
                  {form.cycle_mode === 'on_off' && (
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <Label className="text-xs">Tuần ON</Label>
                        <Input type="number" value={form.cycle_on_weeks} onChange={e => setForm(f => ({ ...f, cycle_on_weeks: Number(e.target.value) }))} min={1} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Tuần OFF</Label>
                ...
                <Button onClick={handleSave} disabled={addSup.isPending || updateSup.isPending} className="w-full">
                  {addSup.isPending || updateSup.isPending ? 'Đang lưu...' : editId ? 'Cập nhật' : 'Thêm'}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
        ...
        {supplements && supplements.length > 0 ? (
            ...
            <motion.div key={s.id} variants={fadeUp} className="glass-card p-4 space-y-2">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {s.cycle_mode === 'on_off' && (
                    <p className="text-[10px] text-metric-purple mt-1">🔄 Cycle: {s.cycle_on_weeks} tuần ON / {s.cycle_off_weeks} tuần OFF</p>
                  )}
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => { deleteSup.mutate(s.id); toast.success('Đã xóa'); }}><Trash2 className="w-3.5 h-3.5 text-muted-foreground" /></Button>
                </div>
              </div>
            </motion.div>
          ))
          ...
          <motion.div variants={fadeUp} className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">Chưa có supplement nào</p>
          </motion.div>
        )}
      </motion.main>
    </div>
