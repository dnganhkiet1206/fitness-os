...
/**
 * `koaStepA` is delayed 1.35s on a 1.5s loop, so at any moment its progress
 * is (t/1.5 − 0.9) mod 1 ≡ t/1.5 + 0.1. A CSS delay runs the clock BACK, so
 * the phase is +0.1, not +0.9 — checked against the design in a browser,
 * where +0.9 puts the left limbs 17° out of step on average.
 */
  ...
  const runningLegs = (
    <>
      <G transform="translate(6,0)">
        <Rot v={run} phase={STEP_A_PHASE} from={-22} to={24} ox={PIVOTS.legLeft.x} oy={PIVOTS.legLeft.y}>
